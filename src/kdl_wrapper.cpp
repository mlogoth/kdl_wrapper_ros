...
KDLWrapper::KDLWrapper(std::string robot_description, ros::NodeHandle nh) : _robot_description(robot_description), _n(nh)
{
...
KDLWrapper::KDLWrapper(KDL::Tree kdl_tree, ros::NodeHandle nh) : _n(nh)
{
    /// Create a KDL Pointer
    _tree_ptr = new KDL::Tree();
    *_tree_ptr = kdl_tree;
...
bool KDLWrapper::init(const std::string &chain_root, const std::string &chain_tip, bool gravity = true)
{
    ...
    // Number of Links
    _nlinks = _kdl_chain.getNrOfSegments();
    ...
    for (unsigned int i = 0; i < _nlinks; i++) //NOTE Remove the origin and the last segments
    {

        if (_kdl_chain.getSegment(i).getJoint().getType() != KDL::Joint::JointType::None)
        {
            _jnt_names[joint_index] = _kdl_chain.getSegment(i).getJoint().getName();
            joint_index ++;
        }
        _link_names[i] = _kdl_chain.getSegment(i).getName();
    }
    // print for
    std::cout << "> KDL: Link Names: " << std::endl;
    for (unsigned int elt = 0; elt<_nlinks; elt++){
        std::cout<< _link_names[elt]<<std::endl;
    }

    std::cout << "> KDL: Joint Names: " << std::endl;
    for (unsigned int elt = 0; elt<_ndof; elt++){
        std::cout<< _jnt_names[elt]<<std::endl;
    }
    ...
    ROS_INFO("KDL Wrapper Initialization complete!");
...
{
    Eigen::Matrix3d m;
    m = Eigen::AngleAxisd(euler[0], Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(euler[1], Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(euler[2], Eigen::Vector3d::UnitZ());
    return m;
}
